...
<xsl:stylesheet
        ...
        <poguesGoto:IdList>
            <xsl:for-each select="//*[@id]">
                ...
                <xsl:if test="local-name(.)='Child' or local-name(.)='Questionnaire' or local-name()='IfThenElse' or local-name()='Loop'">
                    <poguesGoto:idElement id="{./@id}" position="{position()}" type="{local-name(.)}">
                        <poguesGoto:childrenId>
                            <xsl:for-each select="pogues:Child | pogues:IfThenElse | pogues:Loop | pogues:IfTrue/pogues:Child | pogues:IfTrue/pogues:IfThenElse | pogues:IfTrue/pogues:Loop">
                                <poguesGoto:childId>
        ...
        <poguesGoto:GotoList>
            <xsl:for-each select="//pogues:FlowControl[not(@flowControlType) and not(contains(pogues:IfTrue,'-'))]">
                ...
                <poguesGoto:gotoValue start="after" flowid="{@id}">
                    ...
                    <poguesGoto:From id="{parent::pogues:Child/@id}"
            ...
            <xsl:for-each select="$list_goto/poguesGoto:gotoValue">
                ...
                <xsl:if test="(poguesGoto:To/number(@position) - poguesGoto:From/number(@position) &gt; 0)
                    ...
                    <xsl:copy>
                        ...
                        <xsl:copy-of select="poguesGoto:QuestionFilter"/>
                    </xsl:copy>
                </xsl:if>
            </xsl:for-each>
        </poguesGoto:GotoList>
    ...
    <xd:doc>
        ...
        <xd:desc>
            ...
            Goto1 : Expression : not(condition1) ; From : 1 ; To : 6
            Goto2 : Expression : not(condition2) ; From : 2 ; To : 4 (no longer the same as the previous Goto : the split point has changed)
            ...
            <xsl:for-each select="$list_distinct_goto//poguesGoto:gotoValue">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:variable name="initial-filter" select="poguesGoto:QuestionFilter" as="node() *"/>
                        ...
                        <!-- List of the points where to split Goto2 and Expression after each of them : preparing gotoLap -->
                            ...
                            <poguesGoto:gotoValues>
                                <xsl:for-each select="$overlapping-goto/poguesGoto:gotoValue">
                                    <poguesGoto:gotoValue flowid="{@flowid}">
                                        ...
                                        <!-- When a Goto stops inside a Sequence Child that is overlapped by another, the overlap must include the Sequence's label -->
                                        <!-- The point where to start the split is replaced by 
                                             the first ancestor of Goto1.To that doesn't include nor $initial-to nor $initial-from
                                                                        and that parent's Child is an ancestor of $initial-to or $initial-from -->
                                        ...
                                        <xsl:for-each select="poguesGoto:QuestionFilter[@id = $initial-filter//poguesGoto:QuestionFilter/@id]">
                                            <xsl:copy-of select="."/>
                                        </xsl:for-each>
                                    </poguesGoto:gotoValue>
                                </xsl:for-each>
        ...
        <poguesGoto:GotoList>
            <xsl:for-each select="$list_no_overlap_goto//poguesGoto:gotoValue">
                ...
                <xsl:for-each select="$child-tree//poguesGoto:idElement">
                    <xsl:variable name="ancestor-add-condition">
                        <!-- ancestor-or-self ? -->
                        <xsl:for-each select="$initial-filter//poguesGoto:QuestionFilter[@id != current()/ancestor-or-self::poguesGoto:idElement[@type='IfThenElse']/@id]">
                            <xsl:if test="position() != 1">
                                <!-- or ? -->
                                <xsl:value-of select="') and ('"/>
                            </xsl:if>
                            <xsl:value-of select="$root//pogues:IfThenElse[@id = current()/@id]/pogues:Expression"/>
                        </xsl:for-each>
                    </xsl:variable>
                    <xsl:variable name="new-condition">
                        <xsl:choose>
                            <xsl:when test="$ancestor-add-condition = ''">
                                <xsl:value-of select="$initial-condition"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <!-- and ? not ? -->
                                <xsl:value-of select="concat('(',$ancestor-add-condition,') and (',$initial-condition,')')"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:variable>
                    <xsl:choose>
    ...
    <xd:doc>
        <xd:desc>the template for root element</xd:desc>
    </xd:doc>
    <xsl:template match="/pogues:Questionnaire" priority="1">
        ...
        <xsl:for-each select="//pogues:Loop">
            ...
            <xsl:variable name="loop-first-final-descendant-position" select="$child-tree//poguesGoto:idElement[@id=$loop-id]//poguesGoto:idElement[not(descendant::poguesGoto:idElement)][1]/number(@position)"/>
            ...
            <xsl:for-each select="$list_goto//poguesGoto:gotoValue">
                <!-- From inside the loop and To an element after the next element : GoTo the first next element means to the end of the loop  -->
                <!-- From before the loop and To an element inside the loop -->
                <xsl:if test="(poguesGoto:From/number(@position) &gt;= $loop-first-final-descendant-position
                           and poguesGoto:From/number(@position) &lt; $after-loop-position
                           and poguesGoto:To/number(@position) &gt; $after-loop-position)
                       or (poguesGoto:From/number(@position) &lt; $loop-first-final-descendant-position
                           and poguesGoto:To/number(@position) &gt; $loop-first-final-descendant-position
                           and poguesGoto:To/number(@position) &lt; $after-loop-position)">
    ...
    <xsl:template match="*[not(descendant::pogues:Child) and not(descendant::pogues:IfThenElse)] | comment()" mode="first-child-next-brother">
        <xsl:param name="goto-style"/>
        <xsl:param name="stop-position"/>

        <xsl:if test="$stop-position != $last or following-sibling::pogues:Child or following-sibling::*[descendant::pogues:Child or descendant::pogues:IfThenElse]">
            <xsl:copy-of select="."/>
    ...
    <xsl:template match="*[descendant::pogues:Child or descendant::pogues:IfThenElse]" mode="first-child-next-brother">
        <xsl:param name="goto-style"/>
        <xsl:param name="stop-position"/>
    ...
    <xsl:template match="pogues:Child | pogues:IfThenElse | pogues:Loop" priority="1" mode="first-child-next-brother">
        <xsl:param name="goto-style"/>
        <xsl:param name="stop-position"/>
            ...
            <xsl:choose>
                <xsl:when test="$chosen-goto/poguesGoto:gotoValue/@start='before'">
                    ...
                    <xsl:choose>
                        <xsl:when test="$chosen-goto/poguesGoto:To/@position = $last">
                            <xsl:apply-templates select="following-sibling::*[not(name()='pogues:Child') and not(following-sibling::pogues:Child)
                                                                          and not(name()='pogues:IfThenElse') and not(following-sibling::pogues:IfThenElse)
                                                                          and not(name()='pogues:Loop') and not(following-sibling::pogues:Loop)][1]" mode="first-child-next-brother">
                                <xsl:with-param name="stop-position" select="$stop-position"/>
                                <xsl:with-param name="goto-style" select="'none'"/>
                            </xsl:apply-templates>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:apply-templates select="following-sibling::*[@id=$chosen-goto-to-id]" mode="first-child-next-brother">
                                <xsl:with-param name="stop-position" select="$stop-position"/>
                                <xsl:with-param name="goto-style" select="'none'"/>
                            </xsl:apply-templates>
                        </xsl:otherwise>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:if test="$goto-style != 'after'">
                        <xsl:choose>
                            <xsl:when test="descendant::pogues:Child | descendant::pogues:IfThenElse | descendant::pogues:Loop">
                                <xsl:copy>
                                    <xsl:copy-of select="@* | text() | comment() | processing-instruction()"/>
                                    <xsl:apply-templates select="node()[1]" mode="first-child-next-brother">
                                        <xsl:with-param name="goto-style" select="'none'"/>
