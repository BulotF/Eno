...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:eno="http://xml.insee.fr/apps/eno" xmlns:enoxforms="http://xml.insee.fr/apps/eno/form-runner"
    exclude-result-prefixes="xd eno enoxforms" version="2.0">
    ...
    <xsl:template match="SingleResponseQuestion[ancestor::Resource or ancestor::ResourceBind]" mode="model">
        ...
        <xsl:apply-templates select="eno:child-fields($source-context)" mode="source">
            <xsl:with-param name="driver" select="." tunnel="yes"/>
            <xsl:with-param name="question-label" select="eno:serialize(enoxforms:get-label($source-context,$language))" tunnel="yes"/>
        </xsl:apply-templates>
    </xsl:template>
    ...
    <xsl:template match="Instance//DurationDomain | Instance//DateTimeDomain" mode="model" >
        ...
        <xsl:variable name="layout-list" as="node()">
            <xsl:call-template name="dateduration-layout">
                ...
                <xsl:with-param name="format" select="upper-case(enoxforms:get-format($source-context))"/>
                <xsl:with-param name="minimum" select="enoxforms:get-minimum($source-context)"/>
                <xsl:with-param name="maximum" select="enoxforms:get-maximum($source-context)"/>
            </xsl:call-template>
        </xsl:variable>
    ...
    <xsl:template match="Bind//*" mode="model" priority="-1">
        ...
        <xsl:variable name="name" select="enoxforms:get-name($source-context)"/>
        <xsl:variable name="required" select="enoxforms:is-required($source-context)" as="xs:boolean"/>
        <xsl:variable name="relevant" select="enoxforms:get-relevant($source-context)"/>
        <xsl:variable name="variable-calculate" select="enoxforms:get-variable-calculation($source-context)"/>
        ...
        <xsl:variable name="constraint" select="enoxforms:get-constraint($source-context)"/>
        ...
        <xf:bind id="{$name}-bind" name="{$name}" ref="{$name}">
            ...
            <xsl:if test="$variable-calculate != ''">
                <xsl:attribute name="calculate">
                    <xsl:call-template name="replaceVariablesInFormula">
                        ...
                        <xsl:with-param name="variables" as="node()">
                            <Variables>
                                <xsl:for-each select="tokenize(enoxforms:get-variable-calculation-variables($source-context),' ')">
                                    <xsl:sort select="string-length(.)" order="descending"/>
                                    <Variable><xsl:value-of select="."/></Variable>
                                </xsl:for-each>
                            </Variables>
            ...
            <xsl:if test="$constraint != ''">
                <xsl:element name="xf:constraint">
                    <xsl:variable name="alert-level" select="enoxforms:get-alert-level($source-context)"/>
                    <xsl:if test="$alert-level != ''">
                        <xsl:attribute name="level" select="$alert-level"/>
                    </xsl:if>
                    <xsl:attribute name="value">
                        <xsl:if test="self::ConsistencyCheck and enoxforms:get-readonly-ancestors($source-context) != ''">
                            <xsl:variable name="initial-readonly-ancestors">
                                <xsl:for-each select="enoxforms:get-readonly-ancestors($source-context)">
                            ...
                            <xsl:call-template name="replaceVariablesInFormula">
                                ...
                                <xsl:with-param name="variables" as="node()">
                                    <Variables>
                                        <xsl:for-each select="enoxforms:get-readonly-ancestors-variables($source-context)">
                                            <xsl:sort select="string-length(.)" order="descending"/>
                                            <Variable><xsl:value-of select="."/></Variable>
                                        </xsl:for-each>
                                    </Variables>
                        ...
                        <xsl:call-template name="replaceVariablesInFormula">
                            ...
                            <xsl:with-param name="variables" as="node()">
                                <Variables>
                                    <xsl:for-each select="tokenize(enoxforms:get-control-variables($source-context),' ')">
                                        <xsl:sort select="string-length(.)" order="descending"/>
                                        <Variable><xsl:value-of select="."/></Variable>
                                    </xsl:for-each>
                                </Variables>
    ...
    <xsl:template match="Bind//NumericDomain" mode="model">
        ...
        <xsl:variable name="readonly" select="enoxforms:get-readonly($source-context)"/>
        <xsl:variable name="format-constraint" select="enoxforms:get-format-constraint($source-context)"/>
        <xsl:variable name="number-of-decimals" select="enoxforms:get-number-of-decimals($source-context)"/>
        <xsl:variable name="minimum" select="enoxforms:get-minimum($source-context)"/>
        <xsl:variable name="maximum" select="enoxforms:get-maximum($source-context)"/>
            ...
            <xf:bind id="{$business-name}-add-occurrence-bind" ref="{$business-name}-AddOccurrence">
                <xsl:if test="enoxforms:get-maximum-occurrences($source-context)!=''">
                    <xsl:variable name="maximum">
                        <xsl:call-template name="replaceVariablesInFormula">
                            <xsl:with-param name="formula" select="enoxforms:get-maximum-occurrences($source-context)"/>
                            <xsl:with-param name="variables" as="node()">
                                <Variables>
                                    <xsl:for-each select="tokenize(enoxforms:get-maximum-occurrences-variables($source-context),' ')">
                                        <xsl:sort select="string-length(.)" order="descending"/>
                                        <Variable><xsl:value-of select="."/></Variable>
                                    </xsl:for-each>
                                </Variables>
    ...
    <xsl:template match="Bind//TableLoop" mode="model">
        ...
        <xsl:choose>
            ...
            <xsl:when test="number(enoxforms:get-minimum-lines($source-context)) &lt; number(enoxforms:get-maximum-lines($source-context))">
                <xf:bind id="{$business-name}-addline-bind" ref="{$business-name}-AddLine"
                    relevant="count({$instance-ancestor-label}{$container}/{$business-name}) &lt; {enoxforms:get-maximum-lines($source-context)}"/>
            </xsl:when>
            <xsl:otherwise/>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="Bind//DurationDomain | Bind//DateTimeDomain" mode="model">
        ...
        <xsl:variable name="relevant" select="enoxforms:get-relevant($source-context)"/>
        <xsl:variable name="readonly" select="enoxforms:get-readonly($source-context)"/>
        <xsl:variable name="dateduration-format" select="upper-case(enoxforms:get-format($source-context))"/>
        <xsl:variable name="minimum" select="enoxforms:get-minimum($source-context)"/>
        <xsl:variable name="maximum" select="enoxforms:get-maximum($source-context)"/>
        <xsl:variable name="layout-list" as="node()">
            <xsl:call-template name="dateduration-layout">
    ...
    <xsl:template match="Resource//*[not(ancestor::ResourceItem)]" mode="model" priority="-1">
        ...
        <xsl:variable name="hint" select="enoxforms:get-hint($source-context, $language)"/>
    ...
    <xsl:template match="Resource//ConsistencyCheck" mode="model">
        ...
        <xsl:variable name="alert" select="eno:serialize(enoxforms:get-label($source-context, $language))"/>
    ...
    <xsl:template match="ResourceItem//Code[ancestor::CodeDomain or ancestor::BooleanDomain]" mode="model">
        ...
        <item>
            ...
            <value>
                <xsl:value-of select="enoxforms:get-value($source-context)"/>
            </value>
        </item>
    </xsl:template>
            ...
            <xsl:element name="{@variable}">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$dateduration-format = 'YYYY-MM-DD' or upper-case($dateduration-format) = 'JJ/MM/AAAA'">
                        <alert>
                            <xsl:value-of select="enoxforms:get-alert($source-context, $language)"/>
                        </alert>
                    </xsl:when>
            ...
            <xf:label>
                <xsl:attribute name="ref">
                    <xsl:call-template name="label-ref-condition">
                        ...
                        <xsl:with-param name="conditioning-variables" select="enoxforms:get-label-conditioning-variables($source-context, $languages[1])"/>
                        <xsl:with-param name="instance-ancestor" select="$instance-ancestor"/>
                    </xsl:call-template>
                </xsl:attribute>
            </xf:label>
    ...
    <xsl:template match="Body//SubModule | Body//Group" mode="model">
        ...
        <xsl:variable name="name" select="enoxforms:get-name($source-context)"/>
        <xsl:variable name="label" select="enoxforms:get-label($source-context, $languages[1])"/>
        <xsl:variable name="css-class" select="enoxforms:get-css-class($source-context)"/>
    ...
    <xsl:template match="Body//SingleResponseQuestion" mode="model">
        ...
        <xsl:apply-templates select="eno:child-fields($source-context)" mode="source">
            <xsl:with-param name="driver" select="." tunnel="yes"/>
            <xsl:with-param name="question-label" select="eno:serialize(enoxforms:get-label($source-context,$languages[1]))" tunnel="yes"/>
            <xsl:with-param name="question-label-variables" select="enoxforms:get-label-conditioning-variables($source-context, $languages[1])" as="xs:string *" tunnel="yes"/>
            <xsl:with-param name="rich-question-label" select="eno:is-rich-content(enoxforms:get-label($source-context, $languages[1]))" tunnel="yes" as="xs:boolean"/>
        </xsl:apply-templates>
    </xsl:template>
    ...
    <xsl:template match="Body//*" mode="model" priority="-1">
        ...
        <xsl:variable name="name" select="enoxforms:get-name($source-context)"/>
        <xsl:variable name="appearance" select="enoxforms:get-appearance($source-context)"/>
        <xsl:variable name="css-class" select="enoxforms:get-css-class($source-context)"/>
        <xsl:variable name="length" select="enoxforms:get-length($source-context)"/>
        <xsl:variable name="suffix" select="enoxforms:get-suffix($source-context, $languages[1])"/>
        <xsl:variable name="label" select="enoxforms:get-label($source-context, $languages[1])"/>
        <xsl:variable name="hint" select="enoxforms:get-hint($source-context, $languages[1])"/>
        <xsl:variable name="help" select="enoxforms:get-help($source-context, $languages[1])"/>
        <xsl:variable name="alert" select="enoxforms:get-alert($source-context, $languages[1])"/>
        <xsl:variable name="instance-ancestor-label">
            <xsl:value-of select="'instance(''fr-form-instance'')//'"/>
            <xsl:for-each select="tokenize($instance-ancestor,' ')">
                <xsl:value-of select="concat(.,'[@occurrence-id = current()/ancestor::',.,'/@occurrence-id]//')"/>
    ...
    <xsl:template match="Body//ConsistencyCheck" mode="model">
        ...
        <xsl:element name="xf:output">
            ...
            <xf:alert>
                ...
                <xsl:if test="eno:is-rich-content(enoxforms:get-label($source-context, $languages[1]))">
                    <xsl:attribute name="mediatype">text/html</xsl:attribute>
                </xsl:if>
            </xf:alert>
        </xsl:element>
    ...
    <xsl:template match="Rich-Body//Code[ancestor::CodeDomain or ancestor::BooleanDomain]" mode="model" priority="1">
        ...
        <xsl:if test="enoxforms:get-image($source-context) != '' or eno:is-rich-content(enoxforms:get-label($source-context, $languages[1]))">
            <xsl:attribute name="mediatype">text/html</xsl:attribute>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="Body//Table" mode="model">
        ...
        <xsl:variable name="isLongTable">
            <xsl:if test="count(enoxforms:get-body-lines($source-context))>=$lengthOfLongTable">
                <xsl:value-of select="'long-table'"/>
            </xsl:if>
        </xsl:variable>

        <xhtml:table name="{enoxforms:get-name($source-context)}">
            <xsl:choose>
            ...
            <xhtml:colgroup>
                <xhtml:col span="{count(enoxforms:get-header-columns($source-context))}"/>
            </xhtml:colgroup>
            <xhtml:thead>
                <xsl:for-each select="enoxforms:get-header-lines($source-context)">
                    <xhtml:tr>
    ...
    <xsl:template match="Body//TableLoop" mode="model">
        ...
        <xsl:variable name="table-name" select="enoxforms:get-name($source-context)"/>
        <xsl:variable name="loop-name" select="enoxforms:get-business-name($source-context)"/>
        ...
        <xsl:variable name="max-lines" select="enoxforms:get-maximum-lines($source-context)"/>

        <xsl:if test="not($max-lines != '') or number($max-lines) &gt; number(enoxforms:get-minimum-lines($source-context))">
            <xsl:variable name="container" select="enoxforms:get-container-name($source-context)"/>
    ...
    <xsl:template match="Body//FixedCell" mode="model">
        ...
        <xsl:variable name="conditioning-variables" as="xs:string*">
            <xsl:sequence select="enoxforms:get-label-conditioning-variables($source-context, $languages[1])"/>
            <xsl:sequence select="enoxforms:get-cell-value-variables($source-context)"/>
        </xsl:variable>

        <xhtml:td colspan="{enoxforms:get-colspan($source-context)}" rowspan="{enoxforms:get-rowspan($source-context)}">
            <xf:output id="{$name}-control" bind="{$name}-bind">
                <xsl:if test="$css-class != ''">
                    <xsl:attribute name="class" select="$css-class"/>
                </xsl:if>
    ...
    <xsl:template match="Body//QuestionLoop" mode="model">
        ...
        <xsl:param name="languages" tunnel="yes"/>
        <xsl:param name="instance-ancestor" tunnel="yes"/>
        <xsl:variable name="loop-name" select="enoxforms:get-name($source-context)"/>
        <xsl:variable name="container-name" select="enoxforms:get-container-name($source-context)"/>
        <xsl:variable name="business-name" select="enoxforms:get-business-name($source-context)"/>
        <xsl:variable name="instance-ancestor-label">
            <xsl:value-of select="'instance(''fr-form-instance'')//'"/>
                ...
                <xsl:for-each select="$conditioning-variables">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="enoxforms:get-conditioning-variable-formula($source-context,$conditioning-variable) != ''">
                            ...
                            <xsl:call-template name="replaceVariablesInFormula">
                                <xsl:with-param name="formula" select="normalize-space(enoxforms:get-conditioning-variable-formula($source-context,$conditioning-variable))"/>
                                <xsl:with-param name="variables" as="node()">
                                    <Variables>
                                        <xsl:for-each select="tokenize(enoxforms:get-conditioning-variable-formula-variables($source-context,$conditioning-variable),' ')">
                                            <xsl:sort select="string-length(.)" order="descending"/>
                                            <Variable><xsl:value-of select="."/></Variable>
                                        </xsl:for-each>
                                    </Variables>
                        ...
                        <xsl:otherwise>
                            <xsl:variable name="variable-ancestors" select="enoxforms:get-variable-business-ancestors($source-context,$conditioning-variable)"/>
                            <xsl:variable name="variable-type" select="enoxforms:get-variable-representation($source-context,$conditioning-variable)"/>
